...
const AnalysisResult = ({ data }: AnalysisResultProps) => {
  ...
  const getPriceColor = (rating: string) => {
    switch (rating) {
      ...
      case "overpriced": return "stat-negative";
      default: return "stat-warning";
    }
  };
  ...
  return (
    <div className="space-y-6 animate-slide-up">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="text-center p-4 rounded-xl bg-secondary/30">
            ...
            <span className={`text-xs px-2 py-1 rounded-full ${
              data.marketAnalysis.priceRating === "good" 
                ? "bg-primary/20 text-primary" 
                : data.marketAnalysis.priceRating === "overpriced"
            ...
            >
              <div className="flex items-center gap-3">
                <div className={`w-2 h-2 rounded-full ${
                  ...
                  item.urgency === "medium" ? "bg-warning" : "bg-muted-foreground"
                }`} />
      ...
      <Card className={`glass-card p-6 border-2 ${
        data.profitability.isProfitable ? "border-primary/50" : "border-destructive/50"
      }`}>
        <div className="flex items-start gap-4">
          <div className={`p-3 rounded-xl ${
            data.profitability.isProfitable ? "bg-primary/20" : "bg-destructive/20"
          }`}>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {data.videos.map((video, index) => (
              <a
                ...
                className="group relative aspect-video rounded-lg overflow-hidden bg-secondary"
              >
                <img 
                  src={video.thumbnail} 
                  alt={video.title}
