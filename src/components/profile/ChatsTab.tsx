...
import { Plus, Trash2, Calendar, MessageSquare, Pencil, Check, X } from "lucide-react";
...
const ChatsTab = ({
  ...
  const fetchCredits = async () => {
    ...
    const {
      data
    } = await supabase.from("user_credits").select("chat_messages").eq("user_id", user.id).maybeSingle();
  ...
  return <div className="space-y-4">
      {chatCredits > 0 && <Button onClick={handleNewChat} className="w-full bg-primary hover:bg-primary/90">
              ...
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4 min-w-0 flex-1">
                    <div className="p-2 rounded-lg flex-shrink-0 bg-background border-primary border">
                      <MessageSquare className="w-5 h-5 text-primary" />
                    </div>
                    <div className="min-w-0 flex-1">
                      {editingId === chat.id ? <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
