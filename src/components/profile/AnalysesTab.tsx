...
const AnalysesTab = ({
  ...
  const fetchAnalyses = async () => {
    ...
    const {
      data,
      error
    } = await supabase.from("analysis_history").select("*").eq("user_id", user.id).order("created_at", {
      ascending: false
    });
    if (error) {
  ...
  const handleSaveRename = async (e: React.MouseEvent, id: string) => {
    ...
    if (error) {
      ...
      setAnalyses(analyses.map(a => a.id === id ? {
        ...a,
        custom_title: trimmed || null
      } : a));
    }
  ...
  const convertToAnalysisData = (analysis: Analysis) => {
    ...
    return {
      ...
      marketAnalysis: {
        ...
        priceRating: (analysis.price_rating?.toLowerCase() === "good" || analysis.price_rating?.toLowerCase() === "gera" ? "good" : analysis.price_rating?.toLowerCase() === "overpriced" || analysis.price_rating?.toLowerCase() === "aukšta" ? "overpriced" : "average") as "good" | "average" | "overpriced",
        estimatedResaleValue: analysis.estimated_resale_value || 0,
        resaleTimeframe: "6-12 mėn."
      },
      repairEstimate: {
        totalCost: analysis.repair_total_cost || 0,
        items: repairItems as Array<{
          name: string;
          cost: number;
  ...
  if (selectedAnalysis) {
    return <div className="space-y-4">
        <Button variant="outline" onClick={() => setSelectedAnalysis(null)} className="mb-4">
          ← {t("back")}
        </Button>
  ...
  if (analyses.length === 0) {
    return <div className="text-center py-12 border-primary border rounded-xl">
  ...
  return <div className="space-y-3">
      {analyses.map(analysis => <Card key={analysis.id} className="bg-zinc-900 border-primary/20 hover:border-primary/40 transition-colors cursor-pointer" onClick={() => setSelectedAnalysis(analysis)}>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div className="flex items-start sm:items-center gap-3 sm:gap-4 min-w-0">
                ...
                <div className="min-w-0 flex-1">
                  {editingId === analysis.id ? <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
                  ...
                  if (e.key === "Escape") setEditingId(null);
                    ...
                    </div> : <h4 className="font-medium text-foreground text-sm sm:text-base truncate">
                  ...
                  <div className="flex flex-wrap items-center gap-2 sm:gap-3 mt-1">
                    ...
                    {analysis.current_price && <span className="text-xs sm:text-sm text-primary font-medium">
                        {analysis.current_price.toLocaleString()} €
                      </span>}
                    {analysis.price_rating && <Badge className={`${getPriceRatingColor(analysis.price_rating)} text-xs`}>
              ...
              <div className="flex items-center gap-1 self-end sm:self-auto flex-shrink-0">
                <Button variant="ghost" size="sm" onClick={e => handleStartRename(e, analysis)} className="text-muted-foreground hover:text-foreground">
                  <Pencil className="w-3.5 h-3.5" />
                </Button>
                {analysis.listing_url && <Button variant="ghost" size="sm" onClick={e => {
              e.stopPropagation();
