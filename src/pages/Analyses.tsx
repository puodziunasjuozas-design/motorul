...
const Analyses = () => {
  ...
  const fetchCredits = async () => {
    ...
    const {
      data
    } = await supabase.from("user_credits").select("analysis_credits").eq("user_id", user.id).single();
  ...
  return <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 sm:px-6 pt-24 sm:pt-28 pb-12 sm:pb-20">
        {showAnalysisTool ? <AnalysisTool onClose={() => {
        ...
        setRefreshKey(prev => prev + 1);
            ...
            <h2 className="text-foreground text-2xl mb-4 text-center font-extrabold sm:text-5xl">{t("analysisHistory")}</h2>
            <div className="border border-primary/30 rounded-lg p-4 sm:p-6 bg-background">
              {analysisCredits > 0 && <Button variant="hero" size="lg" className="w-full mb-6" onClick={() => setShowAnalysisTool(true)}>
